...
export default function App() {
  ...
  users.forEach(user => {
    let findUser = sectionUsers.find(sectionUser => sectionUser.userId == user.userId);

    if (findUser) {
      findUser.data.push(user.title);
    } else {
      sectionUsers.push({
        userId: user.userId,
        data: [user.title]//["ksndjandjas","ansjandjas","nskan"]
      })
    }
  })
  ...
  return (
    <View>
      ...
      <SectionList
        ...
        renderItem={({ data: { item } }) => <Text style={{
          color: 'red'
        }}>{item}</Text>}
        renderSectionHeader={({ section: { userId } }) => {
          return <Text style={{ fontSize: 25 }}>{userId}</Text>
        }}
      />
